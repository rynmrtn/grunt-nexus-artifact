...
module.exports = (grunt) ->
	...
	grunt.registerTask 'nexus', 'Download an artifact from nexus', ->
		done = @async()
		...
		# get the config for our task
		config = grunt.config.get('nexus')
		downloads = []

		_.each config.fetch, (cfg) ->
			...
			downloads.push util.download(artifact, cfg.path)

		Q.all(downloads).then(() ->
			done()
		).fail (err) ->
			grunt.fail.warn err